...
$iGeneralJournalId = 1; //$_REQUEST['id'];
$iGJ_Date = "2022-06-21"; //$_REQUEST['trans_date'];
if ($iGeneralJournalId) {
    $_SESSION['gj_id'] = $iGeneralJournalId;
    $_SESSION['trans_date'] = $iGJ_Date;
}
$time = time();
//$config = @include('filemanager.config.config');
...
?>
<link href="{{ asset('global_assets/css/style.css') }}" rel="stylesheet" type="text/css" />
<link href="{{ asset('global_assets/js/jPlayer/skin/blue.monday/jplayer.blue.monday.css') }}" rel="stylesheet" type="text/css">
<script src="{{ asset('global_assets/js/plugins.js') }}"></script>
<script src="{{ asset('global_assets/js/jPlayer/jquery.jplayer/jquery.jplayer.js') }}"></script>
<script src="{{ asset('global_assets/js/modernizr.custom.js') }}"></script>
<?php
if ($aviary_active) {
...
dictInvalidFileType: "<?php echo trans('Error_extension'); ?>",
        ...
        url: "fin-general-ledgers.upload",
...
<script src="{{ asset('global_assets/js/include.js') }}"></script>
...
if ($ftp) {
    ...
    if (!file_exists($current_path . $rfm_subfolder . $subdir)) {
        mkdir($current_path . $rfm_subfolder . $subdir, 0777, true);
    }
...
<?php if ($upload_files) { ?>
                                ...
                                <?php if ($create_folders /* && $objEmployee->iEmployeeTypeId == 1 */) { ?>
...
<?php if ($_GET['type'] != 1 && $_GET['type'] != 3 && $show_filter_buttons) { ?>
    ...
    <div class="row-fluid">
<?php
$link = "filemanager/?" . $get_params;
?>
        <ul class="breadcrumb">
            <li class="pull-left"><i class="icon-home"></i></li>
...
if (!empty($bc))
    foreach ($bc as $k => $b) {
        ...
        if ($k == count($bc) - 2) {
            ...
            <?php } ?>
            <li class="pull-right"><a id="refresh" class="btn-small" href="filemanager/?<?php echo $get_params . $subdir . "&" . uniqid() ?>"><i class="icon-refresh"></i></a></li>
...
<?php } else { ?>
    ...
    <?php if ($show_sorting_bar) { ?>
        ...
        <?php } ?>
                                <a class="folder-link" href="filemanager/?<?php echo $get_params . rawurlencode($src) . "&" . ($callback ? 'callback=' . $callback . "&" : '') . uniqid() ?>">
        ...
        <?php } else { ?>
                ...
                echo "ellipsis";
            } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="filemanager/?<?php echo $get_params . rawurlencode($src) . "&" . uniqid() ?>"><?php echo $file; ?></a></h4>
                                    </div>
                    ...
                    foreach ($files as $nu => $file_array) {
                        ...
                        if ((!($_GET['type'] == 1 && !$is_img) && !(($_GET['type'] == 3 && !$is_video) && ($_GET['type'] == 3 && !$is_audio))) && $class_ext > 0) {
                            ...
                            //This is where
                            if ($objGeneral->fnGet("v_status") && ($objGeneral->fnGet("v_status") != 0 && $objGeneral->fnGet("v_status") != 6)) {
